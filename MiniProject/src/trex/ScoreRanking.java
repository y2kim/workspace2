...
public class ScoreRanking {
	...
	public void readTxt() {	// 텍스트파일 읽어와서 리스트에 저장
		try(BufferedReader br = new BufferedReader(new FileReader("src/tetris/resources/TetrisRanking.txt"));) {
			rankList.clear();
			while(true) {
	...
	public void renewRank(int score, String name) {	// 랭크 갱신 
			...
			for(int i = 0; i < size; i++) {
				if(score < rankList.get(i).getScore()) {	
					...
					while(j < size) {
						...
						System.out.println(rankList.get(j).getName() + ", " + rankList.get(j).getScore() );
						j++;
						loop = false;
		...
		for(int i = 0; i < tempSize; i++) {
			...
			//System.out.println(rankList.get(i).getName() + ", " + rankList.get(i).getScore() );
			
		}
	}
	
	public void saveTxt() {	// 파일로 내보내기 
		file = new File("src/tetris/resources/TetrisRanking.txt");
		try(BufferedWriter bw = new BufferedWriter(new FileWriter(file));) {
			...
			NewGameOverDialog.start = false;
		} catch(Exception e) {
			e.printStackTrace();
		}
	}
	
//	public boolean check(Score rank, Score temp) {
//		if(rank.getName().equals(temp.getName()) && (rank.getScore() == temp.getScore())) {
//			return false;
//		} else {
//			return true;
//		}
